...
// count
assert(engine.count() === 446, `count() === 446 (got ${engine.count()})`);

// listCommandNames
const names = engine.listCommandNames();
assert(Array.isArray(names) && names.length === 446, "listCommandNames() returns 446 names");
...
// clearCache
engine.clearCache();
assert(engine.count() === 446, "count() still 446 after clearCache");
